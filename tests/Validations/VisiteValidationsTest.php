...
class VisiteValidationsTest extends KernelTestCase{
    ...
    public function testNonValidNoteVisite(){
        $this ->assertErrors($this->getVisite()->setNote(21), 1, "21 devrait echouer");
        $this ->assertErrors($this->getVisite()->setNote(-1), 1, "-1 devrait echouer");
        $this ->assertErrors($this->getVisite()->setNote(-5), 1, "-5 devrait echouer");
        $this ->assertErrors($this->getVisite()->setNote(25), 1, "25 devrait echouer");
    }
    public function testValidTempMaxVisite(){
        $this -> assertErrors($this->getVisite()->setTempmin(18) ->setTempmax(20),0, "min=18, max=20 devrait reussir");
        $this -> assertErrors($this->getVisite()->setTempmin(19) ->setTempmax(20),0, "min=19, max=20 devrait reussir");
    }
    ...
    public function testValidDatecreationVisite(){
        ...
        $this->assertErrors($this->getVisite()->setDatecreation($aujourdhui), 0, "aujourd'hui devrait reussir");
    ...
    public function testNonValidDatecreationVisite(){
        ...
        $this ->assertErrors($this->getVisite()->setDatecreation($demain), 1, "demain devrait echouer");
        $plustard = (new \DateTime())->add(new \DateInterval ("P5D"));
        $this ->assertErrors($this->getVisite()->setDatecreation($plustard), 1, "plus tard devrait echouer");
    }
    
    public function assertErrors(Visite $visite, int $nbErreurAttendues, string $message=""){
        self::bootKernel();
        $validator = self::getContainer()->get(ValidatorInterface::class);
        $error = $validator->validate($visite);
        $this->assertCount($nbErreurAttendues, $error, $message);
    }
}